...
const VERTEX_ATTRIBUTES: [egui_wgpu::wgpu::VertexAttribute; 2] =
    egui_wgpu::wgpu::vertex_attr_array![0 => Float32x3, 1 => Float32x3];
...
const LINE_ATTRIBUTES: [egui_wgpu::wgpu::VertexAttribute; 2] =
    egui_wgpu::wgpu::vertex_attr_array![0 => Float32x3, 1 => Float32x3];
...
fn bounds_from_positions(positions: &[[f32; 3]]) -> ([f32; 3], [f32; 3]) {
...
fn build_vertices(mesh: &RenderMesh) -> Vec<Vertex> {
...
fn normals_vertices(vertices: &[Vertex], length: f32) -> Vec<LineVertex> {
...
fn bounds_vertices(min: [f32; 3], max: [f32; 3]) -> Vec<LineVertex> {
...
fn grid_and_axes() -> (Vec<LineVertex>, Vec<LineVertex>) {
    let grid_size = 10.0;
    let divisions = 20;
